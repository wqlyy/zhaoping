...
function zlzp(html) {
    ...
    $("table").each(function (i, e) {
        if (i === 0) {
            ...
            var obj = {};
            obj.title = $(e).find('.zwmc').text().trim();
            obj.company = $(e).find('.gsmc').text().trim()
            obj.salary = $(e).find('.zwyx').text().trim()
            obj.city = $(e).find('.gzdd').text().trim()
            obj.url = $(e).find('.zwmc').find('a').eq(0).attr('href');
            obj.pubdate = $(e).find('.gxsj').text().trim();
            data.push(obj)
        }
    })
    return data
...
function ntw(html) {
    ...
    $('.list-items').eq(0).find('li').each(function (index, element) {
        ...
        info.url = "http://www.neitui.me"+$jobInfo.find('.mt5 a').attr('href')
        info.pubdate = $jobInfo.find('.mt5 .ml10').text().trim()
        info.salary = $jobInfo.find('.mr10').text().trim()
        info.city = $jobInfo.find('.mr10').next().text().trim()
